...
describe('parseRetryAfter', () => {
  ...
  it('returns undefined for RFC 850 date format', () => {
    assert.equal(parseRetryAfter('Wednesday, 01-Jan-20 00:00:00 GMT'), undefined);
  });

  it('returns undefined for asctime date format', () => {
    assert.equal(parseRetryAfter('Wed Jan  1 00:00:00 2020'), undefined);
  });

  it('returns undefined for ISO 8601 date format', () => {
    assert.equal(parseRetryAfter('2020-01-01T00:00:00Z'), undefined);
  });

  it('returns undefined for locale-specific date format', () => {
    assert.equal(parseRetryAfter('January 1, 2020'), undefined);
  });

  it('returns undefined for future ISO 8601 date', () => {
    const futureISO = new Date(Date.now() + 60_000).toISOString();
    assert.equal(parseRetryAfter(futureISO), undefined);
  });
...
describe('createRateLimitInterceptor', () => {
  describe('factory validation', () => {
    ...
    it('throws TypeError when options is a number', () => {
      assert.throws(() => createRateLimitInterceptor(1), {
        name: 'TypeError',
    ...
    it('throws TypeError when options is a string', () => {
      assert.throws(() => createRateLimitInterceptor('test'), {
        name: 'TypeError',
        message: 'options must be an object'
      });
    });
    ...
    it('throws TypeError when options is a boolean', () => {
      assert.throws(() => createRateLimitInterceptor(true), {
        name: 'TypeError',
        message: 'options must be an object'
      });
    });
    ...
    it('throws TypeError when options is null', () => {
      assert.throws(() => createRateLimitInterceptor(null), {
        name: 'TypeError',
        message: 'options must be an object'
      });
    });
    ...
    it('throws TypeError when options is an array', () => {
      assert.throws(() => createRateLimitInterceptor([]), {
        name: 'TypeError',
        message: 'options must be an object'
      });
    });
  ...
  describe('response — header parsing', () => {
    ...
    it('ignores negative header values', () => {
      ...
      const seed = createRateLimitResponse({
        headers: {
          'x-ratelimit-limit': '100',
          'x-ratelimit-remaining': '50',
          'x-ratelimit-reset': '1750000000'
      ...
      interceptor.response(ctx1, seed);
