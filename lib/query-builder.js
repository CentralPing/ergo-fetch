...
import {JSON_API_PAGE_STRATEGY_GROUPS} from '@centralping/ergo-wire';
 ...
 */
const PAGE_STRATEGY_GROUPS = JSON_API_PAGE_STRATEGY_GROUPS;
...
/**
 * Rejects mutation methods that conflict with cursor pagination strategy.
 *
 * Per JSON:API, cursor pagination is exclusive — it cannot be combined with
 * sparse fieldsets, includes, filters, or sort parameters on the same request.
 *
 * @param {object} state - Builder state.
 * @throws {TypeError} When cursor pagination is active and conflicting params exist.
 */
function assertCursorExclusivity(state) {
  if (state.pageParams?.cursor != null) {
    throw new TypeError(
      'cursor pagination cannot be combined with fields(), include(), filter(), or sort()'
    );
  }
}

/**
 * Rejects setting cursor pagination when other JSON:API params are already present.
 *
 * @param {object} state - Builder state.
 * @param {object} params - Page parameters being applied.
 * @throws {TypeError} When params include cursor and state has conflicting params.
 */
function assertCursorPageExclusivity(state, params) {
  if (params.cursor == null) return;

  const hasFields = Object.keys(state.fieldSets).length > 0;
  const hasInclude = state.includes != null && state.includes.length > 0;
  const hasFilters = Object.keys(state.filters).length > 0;
  const hasSort = state.sorts != null && state.sorts.length > 0;

  if (hasFields || hasInclude || hasFilters || hasSort) {
    throw new TypeError(
      'page({cursor}) cannot be combined with fields(), include(), filter(), or sort()'
    );
  }
}
...
function createBuilderFromState(state) {
  ...
  builder.page = function page(params) {
    ...
    assertCursorPageExclusivity(state, params);
 ...
 *
 * @param {string} [basePath] - RFC 3986 `path-absolute` for the query (must
 *   start with `/`, contain only unreserved characters, sub-delimiters,
 *   percent-encoded sequences, `:`, `@`, and `/` segment separators).
 * @returns {Readonly<QueryBuilder>} - Frozen immutable query builder.
 * @throws {TypeError} When basePath is provided but is not a valid
...
export function createQueryBuilder(basePath) {
  if (basePath !== undefined) {
    ...
    assertPathAbsolute(basePath, 'basePath');
  }
