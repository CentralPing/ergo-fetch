...
/**
 * Checks whether a string contains ASCII control characters or DEL.
 *
 * The WHATWG URL Standard strips C0 controls (0x00-0x1F), space (0x20),
 * and DEL (0x7F) during parsing. A basePath containing these characters
 * could bypass structural guards (e.g., `'/\t/evil.example'` strips to
 * `'//evil.example'`, escaping the intended host).
 *
 * @param {string} value - String to check.
 * @returns {boolean} - Whether the string contains unsafe characters.
 */
 ...
 *   starting with `/`, without control characters, backslashes, protocol-relative
 *   prefix, query, or fragment).
 * @returns {Readonly<QueryBuilder>} - Frozen immutable query builder.
...
export function createQueryBuilder(basePath) {
  if (basePath !== undefined) {
    ...
    if (
      ...
      basePath.includes('?') ||
      basePath.includes('#') ||
      hasControlCharacter(basePath)
    ) {
      throw new TypeError(
        'basePath must be an absolute path without control characters, backslashes, ' +
          'protocol-relative prefix, query, or fragment'
      );
    }
  }
