...
/**
 * Extracts the path portion from a request target, stripping query and fragment.
 *
 * Splits at the first `?` or `#` delimiter (whichever appears first) so callers
 * can validate the path-absolute portion independently of query/fragment suffixes.
 *
 * @param {string} value - Request target that may include `?query` or `#fragment`.
 * @returns {string} - Path portion before the first delimiter, or the full string.
 */
export function getPathPortion(value) {
  const qIdx = value.indexOf('?');
  const hIdx = value.indexOf('#');

  if (qIdx === -1 && hIdx === -1) return value;
  if (qIdx === -1) return value.slice(0, hIdx);
  if (hIdx === -1) return value.slice(0, qIdx);
  return value.slice(0, Math.min(qIdx, hIdx));
}
