...
/**
 * Parses a Problem Details response body and returns a ProblemDetailsError.
 *
 * The response body is consumed by this call (via `response.json()`). If the
 * body cannot be parsed as JSON, the returned error carries only the HTTP
 * status code.
 *
 * @param {Response} response - Fetch API Response with a Problem Details body.
 * @returns {Promise<ProblemDetailsError>} - Parsed error.
 */
 ...
 *
 * An error is retryable when it is a ProblemDetailsError with a status code
 * in the retryable set (429, 500, 502, 503, 504), or when it is a network
 * error (TypeError thrown by fetch on connection failure).
 *
