...
import {assertPathAbsolute} from './assert-path-absolute.js';
 ...
 * @property {boolean | import('./idempotency.js').IdempotencyInterceptorOptions} [idempotency] -
 *   Idempotency interceptor config (default: disabled, true or object to enable).
 */
 ...
 * @property {string} [idempotencyKey] - Explicit idempotency key for this request.
 */
 ...
 * @property {(path: string, options?: import('./pagination.js').PaginatorOptions) => Readonly<import('./pagination.js').Paginator>} paginate -
 *   Returns an async iterable over paginated responses.
 * @property {(path: string, options?: import('./pagination.js').PaginatorOptions) => Promise<any[]>} paginateAll -
 *   Collects all paginated data into a flattened array.
 * @property {(path?: string) => Readonly<import('./query-builder.js').QueryBuilder>} query -
 *   Creates a JSON:API query builder bound to an optional base path.
 */
...
export function createClient(config) {
  ...
  const idempotencyInterceptor =
    config.idempotency != null && config.idempotency !== false
      ? createIdempotencyInterceptor(normalizeInterceptorOptions(config.idempotency))
      : undefined;
  ...
  /**
   * Returns an async iterable over paginated API responses.
   *
   * Each iteration fetches the next page via the client's `get` method,
   * preserving the full interceptor pipeline. Delegates to `createPaginator`.
   *
   * @param {string} path - Initial request path (must start with /).
   * @param {import('./pagination.js').PaginatorOptions} [options] - Pagination config.
   * @returns {Readonly<import('./pagination.js').Paginator>} - Async iterable over pages.
   */
  client.paginate = function paginate(path, options) {
    return createPaginator(client, path, options);
  };

  /**
   * Collects all paginated data into a single flattened array.
   *
   * Iterates through all pages and concatenates `page.data` arrays. Non-array
   * data values are appended as individual items; `undefined` data is skipped.
   *
   * @param {string} path - Initial request path (must start with /).
   * @param {import('./pagination.js').PaginatorOptions} [options] - Pagination config.
   * @returns {Promise<any[]>} - Flattened array of all items across all pages.
   */
  client.paginateAll = async function paginateAll(path, options) {
    const items = [];

    for await (const page of createPaginator(client, path, options)) {
  ...
  /**
   * Creates an immutable JSON:API query builder.
   *
   * Convenience wrapper around `createQueryBuilder`. The returned builder
   * produces JSON:API-compliant query strings with structural validation.
   *
   * @param {string} [path] - Optional base path for the query.
   * @returns {Readonly<import('./query-builder.js').QueryBuilder>} - Query builder instance.
   */
  client.query = function query(path) {
    return createQueryBuilder(path);
  };

  /**
   * Executes a request through the interceptor pipeline.
   *
   * Network errors (`TypeError` from `fetch()`) are retried when the retry
   * interceptor is active and attempt budget remains. Non-`TypeError` errors
   * (`AbortError`, `TimeoutError`) propagate immediately without retry.
   *
   * @param {string} method - HTTP method.
   * @param {string} path - URL path with optional query/fragment (path portion must be a
   *   valid RFC 3986 path-absolute).
   * @param {RequestOptions} [options] - Per-request options.
   * @returns {Promise<ClientResponse>} - Frozen response object.
   * @throws {TypeError} When path is not a string or the path portion is not a valid
   *   RFC 3986 path-absolute.
   * @throws {TypeError} When body is provided for GET or HEAD requests.
   * @throws {TypeError} When a network error exhausts the retry budget.
  ...
  async function executeRequest(method, path, options) {
    ...
    const pathEnd =
      qIdx === -1 ? (hIdx === -1 ? path.length : hIdx) : hIdx === -1 ? qIdx : Math.min(qIdx, hIdx);
    assertPathAbsolute(path.slice(0, pathEnd), 'path');
    ...
    const resolvedQuery = isQueryBuilder(rawQuery) ? rawQuery.toString() : rawQuery;
    const url = buildUrl(baseUrl, path, options?.params, resolvedQuery);
    ...
    ctx.url = url;
    ctx.baseUrl = baseUrl;
    if (serializedBody !== undefined) ctx.body = serializedBody;
    if (options?.idempotent !== undefined) ctx.idempotent = options.idempotent;
    if (options?.idempotencyKey != null) ctx.idempotencyKey = options.idempotencyKey;
    ...
    for (;;) {
      ...
      try {
        ...
        const retryResult =
          retryInterceptor &&
          activeResponsePipeline.includes(retryInterceptor) &&
          retryInterceptor.error(ctx, err);

        if (retryResult?.retry) {
          if (retryResult.delay > 0) await sleep(retryResult.delay, userSignal);
          continue;
        }
 ...
 * @param {string | object} [query] - Query string or key-value pairs.
