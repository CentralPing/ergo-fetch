 ...
 * @property {number} [timeout] - Default request timeout in milliseconds (default: 30000).
 ...
 * @returns {object} - Frozen null-prototype client instance with HTTP method helpers.
...
export function createClient(config) {
  ...
  async function executeRequest(method, path, options) {
    ...
    const signal = buildAbortSignal(timeout, options?.signal);
    ...
    for (;;) {
      ...
      if (ctx.rateLimitDelay > 0) {
        await sleep(ctx.rateLimitDelay);
