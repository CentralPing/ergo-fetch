...
describe('parseProblemDetails', () => {
  ...
  it('returns undefined for successful responses', async () => {
    ...
    const result = await parseProblemDetails(res);
    assert.equal(result, undefined);
  });

  it('returns undefined for 3xx responses without problem+json', async () => {
    const res = mockResponse({status: 301, statusText: 'Moved Permanently'});

    const result = await parseProblemDetails(res);
    assert.equal(result, undefined);
  });

  it('auto-constructs ProblemDetailsError for 4xx without problem+json', async () => {
    const res = mockResponse({
      status: 404,
      statusText: 'Not Found',
      headers: {'content-type': 'text/plain'}
  ...
  it('auto-constructs ProblemDetailsError for 5xx without problem+json', async () => {
    const res = mockResponse({
      status: 500,
      statusText: 'Internal Server Error',
  ...
  it('auto-constructs ProblemDetailsError for error with no content-type', async () => {
  ...
  it('falls back gracefully when body is a JSON string', async () => {
  ...
  it('falls back gracefully when body is a JSON array', async () => {
  ...
  it('falls back gracefully when body is a JSON number', async () => {
    ...
    assert.equal(err instanceof ProblemDetailsError, true);
    assert.equal(err.status, 503);
    assert.equal(err.title, 'Service Unavailable');
    assert.equal(err.extensions, undefined);
  });

  it('falls back to response.status when body.status is a string', async () => {
    const res = mockResponse({
      status: 503,
      statusText: 'Service Unavailable',
      headers: {'content-type': 'application/problem+json'},
  ...
  it('falls back to response.status when body.status is out of range', async () => {
    const res = mockResponse({
      status: 500,
      statusText: 'Internal Server Error',
      headers: {'content-type': 'application/problem+json'},
      body: {status: 999, title: 'Bad Status'}
    });
  ...
  it('falls back to response.status when body.status is a float', async () => {
    const res = mockResponse({
      status: 422,
      statusText: 'Unprocessable Entity',
      headers: {'content-type': 'application/problem+json'},
      body: {status: 422.5, title: 'Validation Error'}
    });
  ...
  it('prefers problem+json body over auto-construction', async () => {
    const res = mockResponse({
      status: 422,
      statusText: 'Unprocessable Entity',
      headers: {'content-type': 'application/problem+json'},
    ...
    assert.equal(err.detail, 'Field email is required');
  });
});
...
describe('isRetryable', () => {
  ...
  it('returns false for 500 (conditionally retryable, owned by retry interceptor)', () => {
    const err = new ProblemDetailsError({status: 500, title: 'Internal Server Error'});
    assert.equal(isRetryable(err), false);
  });

  it('returns false for 502 (conditionally retryable, owned by retry interceptor)', () => {
    const err = new ProblemDetailsError({status: 502, title: 'Bad Gateway'});
    assert.equal(isRetryable(err), false);
  });

  it('returns false for 504 (conditionally retryable, owned by retry interceptor)', () => {
