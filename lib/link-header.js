 ...
 *
 * Validates content against RFC 9110 §5.6.4:
 * - qdtext = HTAB / SP / %x21 / %x23-5B / %x5D-7E / obs-text
 * - quoted-pair = "\" ( HTAB / SP / VCHAR / obs-text )
 *
 * Returns `undefined` for `value` when the content contains characters
 * outside these grammars or the quoted-string is unterminated.
 *
 * @param {string} str - Input string.
...
function parseQuotedString(str, start) {
  ...
  while (i < str.length) {
    ...
    if (ch === 0x5c && i + 1 < str.length) {
      ...
      // Target must be HTAB / SP / VCHAR / obs-text
    ...
    // qdtext: HTAB / SP / %x21 / %x23-5B / %x5D-7E / obs-text
    // (0x22 and 0x5C are excluded from qdtext but handled by branches above)
  ...
  // Unterminated quoted-string — always invalid
