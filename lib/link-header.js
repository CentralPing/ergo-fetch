 ...
 * `rel`, and any additional link parameters. Parameter names are
 * lowercased per RFC 8288 case-insensitivity.
 *
 ...
 * unclosed brackets) are silently skipped — the parser never throws.
 *
  ...
  while (pos < len) {
    ...
    while (pos < len) {
      ...
      if (pos >= len || headerValue.charCodeAt(pos) !== 0x3d) {
        // Valueless parameter — per RFC 8288 all defined parameters have
        // values, but we store empty string for robustness
        params[name] = '';
        continue;
      }
      ...
      if (pos < len && headerValue.charCodeAt(pos) === 0x22) {
        ...
        params[name] = qs.value;
        pos = qs.end;
      } else {
        const valueEnd = scanToken(headerValue, pos);
        params[name] = headerValue.slice(pos, valueEnd);
        pos = valueEnd;
      }
    }
    ...
    // Resolve relative URIs when a request URL is provided
    let href = rawHref;
    if (requestUrl) {
      try {
        href = new URL(rawHref, requestUrl).href;
      } catch {
        // Invalid URI — preserve raw value
      }
    }
    ...
    for (const rel of rels) {
      ...
      result.set(rel, Object.freeze(link));
    }
  }
