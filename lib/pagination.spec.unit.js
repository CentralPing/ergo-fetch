  ...
  describe('input validation', () => {
    ...
    it('throws TypeError when path does not start with /', () => {
      ...
      assert.throws(() => createPaginator(client, 'items'), {
        name: 'TypeError',
        message: 'path must be path-absolute (start with a single /)'
      });
    });
  ...
  describe('page object shape', () => {
    ...
    it('meta.page reflects starting page when options.page > 1', async () => {
      const {client} = createMockClient([
        {
          status: 200,
          headers: new Headers({link: '</items?page=4&perPage=10>; rel="next"'}),
          body: [{id: 21}],
          url: 'https://api.example.com/items?page=3&perPage=10'
        },
        {
          status: 200,
          headers: new Headers({link: '</items?page=5&perPage=10>; rel="next"'}),
          body: [{id: 31}],
          url: 'https://api.example.com/items?page=4&perPage=10'
        },
        {
          status: 200,
          headers: {},
          body: [{id: 41}],
          url: 'https://api.example.com/items?page=5&perPage=10'
        }
      ]);
      ...
      const {client, calls} = createMockClient([
        {
          status: 200,
          headers: new Headers({
            link: '<https://other.example/items?page=2&perPage=5>; rel="next"'
          }),
          body: [{id: 1}],
          url: 'https://api.example.com/items?page=1&perPage=5'
        },
