...
describe('createClient', () => {
  ...
  describe('request validation', () => {
    ...
    it('throws TypeError for protocol-relative path (//host)', async () => {
      const client = createClient({baseUrl: 'https://api.example.com'});

      await assert.rejects(client.get('//evil.example/resource'), {
        name: 'TypeError',
        message: /RFC 3986/
      });
    });

    it('throws TypeError for path containing backslash', async () => {
      const client = createClient({baseUrl: 'https://api.example.com'});

      await assert.rejects(client.get('/\\evil'), {
        name: 'TypeError',
        message: /RFC 3986/
      });
    });

    it('throws TypeError for path containing control characters', async () => {
      const client = createClient({baseUrl: 'https://api.example.com'});
    ...
    it('throws TypeError for path containing non-ASCII', async () => {
      const client = createClient({baseUrl: 'https://api.example.com'});

      await assert.rejects(client.get('/artícles'), {
        name: 'TypeError',
        message: /RFC 3986/
      });
    });

    it('accepts path with query string', async () => {
    ...
    it('accepts path with fragment', async () => {
      const fetchRef = mockFetch();
      const client = createClient({baseUrl: 'https://api.example.com'});

      await client.get('/items#section');
    ...
    it('accepts path with query string and fragment', async () => {
      const fetchRef = mockFetch();
      const client = createClient({baseUrl: 'https://api.example.com'});

      await client.get('/items?page=2#section');
    ...
    it('rejects invalid path portion even with valid query string', async () => {
      const client = createClient({baseUrl: 'https://api.example.com'});

      await assert.rejects(client.get('//evil.com?foo=bar'), {
        name: 'TypeError',
        message: /RFC 3986/
      });
    });
  ...
  describe('idempotency interceptor integration', () => {
    ...
    it('generates Idempotency-Key when enabled with true', async () => {
      ...
      const headers = fetchRef.mock.calls[0].arguments[1].headers;
      const key = idempotencyHeaderValue(headers);
    ...
    it('passes through explicit idempotencyKey from request options', async () => {
      ...
      const headers = fetchRef.mock.calls[0].arguments[1].headers;
      assert.equal(idempotencyHeaderValue(headers), 'my-explicit-key-123');
      assert.equal(headers.get('idempotency-key'), '"my-explicit-key-123"');
    });

    it('falls back to auto-generation when idempotencyKey is explicitly null', async () => {
      const fetchRef = mockFetch();
      const client = createClient({
        baseUrl: 'https://api.example.com',
        idempotency: true
      ...
      const headers = fetchRef.mock.calls[0].arguments[1].headers;
      const key = idempotencyHeaderValue(headers);
      assert.ok(key, 'should fall back to generated key when idempotencyKey is null');
      assert.match(key, /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/);
      assert.equal(headers.get('idempotency-key'), `"${key}"`);
    });
    ...
    it('reuses key across retries', async () => {
      ...
      const wireHeaders = [];
      mock.method(globalThis, 'fetch', async (url, init) => {
        keys.push(idempotencyHeaderValue(init.headers));
        wireHeaders.push(init.headers.get('idempotency-key'));
        if (keys.length === 1) {
          return new Response(null, {
            status: 503,
            headers: {'content-type': 'text/plain'}
      ...
      const uuidRe = /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/;
      assert.match(keys[0], uuidRe);
      assert.equal(keys[0], keys[1], 'same key must be reused on retry');
      assert.equal(wireHeaders[0], `"${keys[0]}"`);
      assert.equal(wireHeaders[1], `"${keys[1]}"`);
    });
  });
