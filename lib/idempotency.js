 ...
 *
 * Matches ergo `parseIdempotencyKey` acceptance: visible ASCII excluding DQUOTE (`"`)
 * and backslash (`\`), which are escaped at format time.
 *
...
function assertValidIdempotencyKeyValue(key) {
  for (let i = 0; i < key.length; i++) {
    ...
    throw new TypeError(
      'idempotency key must be an RFC 8941 sf-string value (visible ASCII excluding DQUOTE and backslash)'
    );
  }
}
