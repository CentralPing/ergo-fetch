...
export function createIdempotencyInterceptor(options) {
  ...
  /**
   * Serializes explicit-key fingerprint registry access per key across the
   * async `fingerprint()` boundary.
   *
   * @param {string} key - Explicit idempotency key.
   * @param {() => Promise<void>} fn - Critical section covering fingerprint,
   *   registry check/set, and header attach.
   * @returns {Promise<void>}
   */
  async function withExplicitKeyLock(key, fn) {
    const prior = keyTails.get(key) ?? Promise.resolve();
    /** @type {undefined | (() => void)} */
    let release;
    ...
    keyTails.set(key, chained);
    await prior;
    try {
      await fn();
    } finally {
      release?.();
      if (keyTails.get(key) === chained) {
        keyTails.delete(key);
      }
    }
  }
  ...
  /**
   * Stores an explicit key in the fingerprint registry and attaches the header.
   *
   * @param {object} ctx - Request context.
   * @param {string} key - Resolved explicit idempotency key.
   * @param {number} now - Current timestamp in milliseconds.
   * @returns {Promise<void>}
   */
  async function registerExplicitKey(ctx, key, now) {
    // RegistryEntry.bodyHash is undefined for bodiless requests (see typedef).
    const bodyHash = ctx.body == null || ctx.body === '' ? undefined : await fingerprint(ctx.body);
  ...
  /**
   * Stores an auto-generated key in the registry and attaches the header.
   *
   * @param {object} ctx - Request context.
   * @param {string} key - Generated idempotency key.
   * @param {number} now - Current timestamp in milliseconds.
   */
  function registerGeneratedKey(ctx, key, now) {
    const existing = registry.get(key);

    if (existing !== undefined && existing.bodyHash !== undefined) {
      throw new TypeError(
        `Idempotency key "${key}" reused with different request body (fingerprint mismatch)`
      );
    }
    ...
    registry.set(key, {bodyHash: undefined, timestamp: now});
    ctx.headers.set(headerName, formatIdempotencyKey(key));
  }
  ...
  interceptor.request = async function request(ctx) {
    ...
    if (explicitKey != null) {
      await withExplicitKeyLock(key, () => registerExplicitKey(ctx, key, now));
      return;
    }

    registerGeneratedKey(ctx, key, now);
  };
