...
/**
 * Validates that a raw idempotency key is encodable as an RFC 8941 sf-string inner value.
 *
 * Matches ergo `parseIdempotencyKey` acceptance after unescaping: visible ASCII
 * (0x20–0x7E). DQUOTE and backslash are permitted in raw keys and escaped at
 * format time.
 *
 * @param {string} key - Raw idempotency key value.
 * @throws {TypeError} When key contains characters outside the sf-string allowlist.
 */
function assertValidIdempotencyKeyValue(key) {
  for (let i = 0; i < key.length; i++) {
    ...
    throw new TypeError('idempotency key must be an RFC 8941 sf-string value (visible ASCII)');
  }
}

/**
 * Formats an idempotency key as an RFC 8941 quoted string for the HTTP header.
 *
 * ergo-router's idempotency middleware requires RFC 8941 structured field syntax.
 *
 * @param {string} key - Raw idempotency key value.
 * @returns {string} - Quoted header value (e.g., `"my-key"`).
 */
function formatIdempotencyKeyHeader(key) {
  assertValidIdempotencyKeyValue(key);
...
/**
 * Computes the SHA-256 hex digest of a string body.
 *
...
export function createIdempotencyInterceptor(options) {
  ...
  interceptor.request = async function request(ctx) {
    ...
    if (existingKey !== undefined) {
      ctx.headers.set(headerName, formatIdempotencyKeyHeader(existingKey));
      return;
    }
    ...
    const bodyHash =
      explicitKey != null && ctx.body !== undefined ? await computeBodyHash(ctx.body) : undefined;
    ...
    ctx.headers.set(headerName, formatIdempotencyKeyHeader(key));
  };
