...
export function createIdempotencyInterceptor(options) {
  ...
  interceptor.request = async function request(ctx) {
    ...
    if (existingKey !== undefined) {
      ctx.headers.set(headerName, existingKey);
      return;
    }
    ...
    const eligible = explicitKey !== undefined || methods.has(ctx.method);
    ...
    const bodyHash = ctx.body !== undefined ? await computeBodyHash(ctx.body) : undefined;

    const existing = registry.get(key);

    if (existing !== undefined && bodyHash !== undefined && existing.bodyHash !== undefined) {
    ...
    ctx.headers.set(headerName, key);
  };
