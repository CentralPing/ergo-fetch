...
describe('parseLinkHeader', () => {
  ...
  describe('malformed entries', () => {
    ...
    it('preserves raw href when requestUrl is provided but URI is truly unparseable', () => {
      const result = parseLinkHeader('<>; rel="next"', 'http://[::1');

      assert.equal(result.size, 1);
      assert.equal(result.get('next').href, '');
    });
  });
