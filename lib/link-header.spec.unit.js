...
describe('parseLinkHeader', () => {
  ...
  describe('multiple rel values', () => {
    ...
    it('normalizes mixed-case rel values to lowercase', () => {
      const result = parseLinkHeader('<https://example.com>; rel="Next"');

      assert.equal(result.has('next'), true);
      assert.equal(result.has('Next'), false);
      assert.equal(result.get('next').rel, 'next');
    });

    it('normalizes multiple mixed-case rels to lowercase', () => {
      const result = parseLinkHeader('<https://example.com>; rel="Start Index"');
  ...
  describe('qdtext and quoted-pair validation', () => {
    it('skips link-value when qdtext contains NUL (0x00)', () => {
      const result = parseLinkHeader('<https://example.com>; rel="next"; title="bad\x00char"');

      assert.equal(result.size, 0);
    });

    it('skips link-value when qdtext contains CR (0x0D)', () => {
      const result = parseLinkHeader('<https://example.com>; rel="next"; title="bad\x0Dchar"');

      assert.equal(result.size, 0);
    });

    it('skips link-value when qdtext contains LF (0x0A)', () => {
      const result = parseLinkHeader('<https://example.com>; rel="next"; title="bad\x0Achar"');

      assert.equal(result.size, 0);
    });

    it('skips link-value when qdtext contains DEL (0x7F)', () => {
      const result = parseLinkHeader('<https://example.com>; rel="next"; title="bad\x7Fchar"');

      assert.equal(result.size, 0);
    });

    it('skips link-value when qdtext contains C0 control (0x01)', () => {
      const result = parseLinkHeader('<https://example.com>; rel="next"; title="bad\x01char"');

      assert.equal(result.size, 0);
    });

    it('skips link-value when quoted-pair target is NUL', () => {
      const result = parseLinkHeader('<https://example.com>; rel="next"; title="bad\\\x00pair"');

      assert.equal(result.size, 0);
    });

    it('skips link-value when quoted-pair target is DEL', () => {
      const result = parseLinkHeader('<https://example.com>; rel="next"; title="bad\\\x7Fpair"');

      assert.equal(result.size, 0);
    });

    it('parses valid link following link with RFC-illegal content', () => {
      const header =
        '<https://example.com/bad>; rel="bad"; title="bad\x00char", ' +
        '<https://example.com/good>; rel="next"';
      const result = parseLinkHeader(header);

      assert.equal(result.size, 1);
      assert.equal(result.has('next'), true);
      assert.equal(result.get('next').href, 'https://example.com/good');
    });

    it('accepts obs-text (0x80-0xFF) in quoted-string', () => {
      const result = parseLinkHeader('<https://example.com>; rel="next"; title="\x80\xFF"');
      const link = result.get('next');

      assert.equal(result.size, 1);
      assert.equal(link.title, '\x80\xFF');
    });

    it('accepts HTAB in quoted-string content', () => {
      const result = parseLinkHeader('<https://example.com>; rel="next"; title="tab\there"');
      const link = result.get('next');

      assert.equal(result.size, 1);
      assert.equal(link.title, 'tab\there');
    });

    it('accepts valid quoted-pair with HTAB target', () => {
      const result = parseLinkHeader('<https://example.com>; rel="next"; title="esc\\\ttab"');
      const link = result.get('next');

      assert.equal(result.size, 1);
      assert.equal(link.title, 'esc\ttab');
    });

    it('accepts valid quoted-pair with SP target', () => {
      const result = parseLinkHeader('<https://example.com>; rel="next"; title="esc\\ space"');
      const link = result.get('next');

      assert.equal(result.size, 1);
      assert.equal(link.title, 'esc space');
    });

    it('accepts valid quoted-pair with VCHAR target', () => {
      const result = parseLinkHeader('<https://example.com>; rel="next"; title="esc\\!bang"');
      const link = result.get('next');

      assert.equal(result.size, 1);
      assert.equal(link.title, 'esc!bang');
    });

    it('accepts valid quoted-pair with obs-text target', () => {
      const result = parseLinkHeader('<https://example.com>; rel="next"; title="esc\\\x80obs"');
      const link = result.get('next');

      assert.equal(result.size, 1);
      assert.equal(link.title, 'esc\x80obs');
    });

    it('skips link-value when rel parameter has invalid qdtext', () => {
      const result = parseLinkHeader('<https://example.com>; rel="ne\x00xt"');

      assert.equal(result.size, 0);
    });

    it('skips link-value with invalid content even when rel was already parsed', () => {
      const result = parseLinkHeader('<https://example.com>; rel="next"; title="inv\x01alid"');
