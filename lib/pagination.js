...
export function createPaginator(client, path, options) {
  ...
  if (strategy === 'offset') {
    ...
    initialQuery = Object.assign(Object.create(null), mergedExtraQuery, {
      page,
      per_page: perPage
    });
  } else {
...
function assertNoPaginatorQueryConflicts(query, strategy) {
  ...
  if (strategy === 'offset') {
    if (query.page !== undefined || query.per_page !== undefined) {
      throw new TypeError(
        'options.query must not include page or per_page when using paginate() offset strategy; use paginator page/perPage options instead'
      );
    }
  } else if (query.limit !== undefined) {
    throw new TypeError(
      'options.query must not include limit when using paginate() cursor strategy; use paginator limit option instead'
    );
  }
}
