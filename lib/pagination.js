 ...
 * @throws {TypeError} When path is not a string starting with /.
 ...
 * @throws {TypeError} When options.maxPages is not a positive number.
...
export function createPaginator(client, path, options) {
  ...
  if (
    ...
    maxPages <= 0 ||
    (maxPages !== Infinity && !Number.isFinite(maxPages))
  ) {
    throw new TypeError('options.maxPages must be a positive number');
  }
  ...
  iterable[Symbol.asyncIterator] = function asyncIterator() {
    return generatePages(client, path, initialQuery, headers, signal, maxPages);
  };
 ...
 */
async function* generatePages(client, initialPath, initialQuery, headers, signal, maxPages) {
  ...
  while (pageNumber < maxPages) {
    ...
    const done = !hasNext || pageNumber >= maxPages;
    ...
    if (!hasNext || pageNumber >= maxPages) return;
