...
describe('createQueryBuilder', () => {
  describe('factory validation', () => {
    ...
    it('accepts path with unreserved characters (dash, dot, underscore, tilde)', () => {
      const builder = createQueryBuilder('/a-b.c_d~e');

      assert.equal(builder.path, '/a-b.c_d~e');
    });

    it('accepts path with sub-delimiters', () => {
      const builder = createQueryBuilder("/a!$&'()*+,;=b");

      assert.equal(builder.path, "/a!$&'()*+,;=b");
    });

    it('accepts path with colon and at-sign (pchar)', () => {
      const builder = createQueryBuilder('/user:admin@host');

      assert.equal(builder.path, '/user:admin@host');
    });

    it('accepts path with valid percent-encoding (uppercase hex)', () => {
      const builder = createQueryBuilder('/articles/%E4%B8%AD%E6%96%87');

      assert.equal(builder.path, '/articles/%E4%B8%AD%E6%96%87');
    });

    it('accepts path with valid percent-encoding (lowercase hex)', () => {
      const builder = createQueryBuilder('/articles/%2f%7e');

      assert.equal(builder.path, '/articles/%2f%7e');
    });

    it('accepts multi-segment path', () => {
      const builder = createQueryBuilder('/api/v2/articles');

      assert.equal(builder.path, '/api/v2/articles');
    });
    ...
    it('throws TypeError when basePath is protocol-relative (authority escape)', () => {
      assert.throws(() => createQueryBuilder('//evil.example/articles'), {
        name: 'TypeError',
        message: /RFC 3986/
      });
    });

    it('throws TypeError when basePath contains a query delimiter', () => {
      assert.throws(() => createQueryBuilder('/articles?stale=true'), {
        name: 'TypeError',
        message: /RFC 3986/
      });
    });

    it('throws TypeError when basePath contains a fragment delimiter', () => {
      assert.throws(() => createQueryBuilder('/articles#section'), {
        name: 'TypeError',
        message: /RFC 3986/
      });
    });

    it('throws TypeError when basePath contains a backslash (authority escape)', () => {
      assert.throws(() => createQueryBuilder('/\\evil.example/articles'), {
        name: 'TypeError',
        message: /RFC 3986/
      });
    ...
    it('throws TypeError when basePath contains a tab (control character host escape)', () => {
      assert.throws(() => createQueryBuilder('/\t/evil.example/articles'), {
        name: 'TypeError',
        message: /RFC 3986/
      });
    ...
    it('throws TypeError when basePath contains a newline (control character host escape)', () => {
      assert.throws(() => createQueryBuilder('/\n/evil.example/articles'), {
        name: 'TypeError',
        message: /RFC 3986/
      });
    });

    it('throws TypeError when basePath contains a space', () => {
      assert.throws(() => createQueryBuilder('/evil path/articles'), {
        name: 'TypeError',
        message: /RFC 3986/
      });
    });

    it('throws TypeError when basePath contains square brackets', () => {
      assert.throws(() => createQueryBuilder('/articles[0]'), {
        name: 'TypeError',
        message: /RFC 3986/
      });
    });

    it('throws TypeError when basePath contains curly braces', () => {
      assert.throws(() => createQueryBuilder('/articles/{id}'), {
        name: 'TypeError',
        message: /RFC 3986/
      });
    });

    it('throws TypeError when basePath contains a pipe', () => {
      assert.throws(() => createQueryBuilder('/a|b'), {
        name: 'TypeError',
        message: /RFC 3986/
      });
    });

    it('throws TypeError when basePath contains a caret', () => {
      assert.throws(() => createQueryBuilder('/a^b'), {
        name: 'TypeError',
        message: /RFC 3986/
      });
    ...
    it('throws TypeError when basePath contains non-ASCII without percent-encoding', () => {
      assert.throws(() => createQueryBuilder('/artícles'), {
        name: 'TypeError',
        message: /RFC 3986/
      });
    });

    it('throws TypeError when basePath has trailing percent sign', () => {
      assert.throws(() => createQueryBuilder('/articles%'), {
        name: 'TypeError',
        message: /RFC 3986/
      });
    ...
    it('throws TypeError when basePath has incomplete percent-encoding (one hex digit)', () => {
      assert.throws(() => createQueryBuilder('/articles%2'), {
        name: 'TypeError',
        message: /RFC 3986/
      });
    });

    it('throws TypeError when basePath has invalid first hex digit in percent-encoding', () => {
      assert.throws(() => createQueryBuilder('/articles%ZZ'), {
        name: 'TypeError',
        message: /RFC 3986/
      });
    });

    it('throws TypeError when basePath has invalid second hex digit in percent-encoding', () => {
      assert.throws(() => createQueryBuilder('/articles%2G'), {
        name: 'TypeError',
        message: /RFC 3986/
      });
    });
  ...
  describe('filter()', () => {
    ...
    it('throws TypeError when a filter value is NaN', () => {
      const builder = createQueryBuilder();

      assert.throws(() => builder.filter({count: NaN}), {
        name: 'TypeError',
        message: /must be a finite number/
      });
    });

    it('throws TypeError when a filter value is Infinity', () => {
      const builder = createQueryBuilder();

      assert.throws(() => builder.filter({count: Infinity}), {
        name: 'TypeError',
        message: /must be a finite number/
      });
    });
  ...
  describe('page()', () => {
    ...
    it('throws TypeError when mixing size and cursor strategies', () => {
      const builder = createQueryBuilder();

      assert.throws(() => builder.page({size: 10, cursor: 'abc'}), {
        name: 'TypeError',
        message: /mixes pagination strategies/
      });
    });

    it('throws TypeError when mixing number and limit strategies', () => {
      const builder = createQueryBuilder();

      assert.throws(() => builder.page({number: 1, limit: 10}), {
        name: 'TypeError',
        message: /mixes pagination strategies/
      });
    });
    ...
    it('throws TypeError when a page value is NaN', () => {
      const builder = createQueryBuilder();

      assert.throws(() => builder.page({number: NaN}), {
        name: 'TypeError',
        message: /must be a finite number/
      });
    });

    it('throws TypeError when a page value is Infinity', () => {
      const builder = createQueryBuilder();

      assert.throws(() => builder.page({limit: Infinity}), {
        name: 'TypeError',
        message: /must be a finite number/
      });
    });
    ...
    it('throws TypeError when adding fields after cursor pagination', () => {
      const builder = createQueryBuilder().page({cursor: 'abc'});

      assert.throws(() => builder.fields('articles', ['title']), {
        name: 'TypeError',
        message: /cursor pagination cannot be combined/
      });
    });

    it('throws TypeError when adding filter after cursor pagination', () => {
      const builder = createQueryBuilder().page({cursor: 'abc'});

      assert.throws(() => builder.filter({published: true}), {
        name: 'TypeError',
        message: /cursor pagination cannot be combined/
      });
    });

    it('throws TypeError when adding include after cursor pagination', () => {
      const builder = createQueryBuilder().page({cursor: 'abc'});
    ...
    it('throws TypeError when adding sort after cursor pagination', () => {
      const builder = createQueryBuilder().page({cursor: 'abc'});

      assert.throws(() => builder.sort(['-createdAt']), {
        name: 'TypeError',
        message: /cursor pagination cannot be combined/
      });
    });

    it('throws TypeError when setting cursor page on builder with fields', () => {
      const builder = createQueryBuilder().fields('articles', ['title']);

      assert.throws(() => builder.page({cursor: 'abc'}), {
        name: 'TypeError',
        message: /page\(\{cursor\}\) cannot be combined/
      });
    });

    it('throws TypeError when setting cursor page on builder with include', () => {
      const builder = createQueryBuilder().include(['author']);
    ...
    it('throws TypeError when setting cursor page on builder with filter', () => {
      const builder = createQueryBuilder().filter({published: true});
    ...
    it('throws TypeError when setting cursor page on builder with sort', () => {
      const builder = createQueryBuilder().sort(['-createdAt']);
  ...
  describe('param()', () => {
    ...
    it('throws TypeError when value is NaN', () => {
      const builder = createQueryBuilder();

      assert.throws(() => builder.param('camelCase', NaN), {
        name: 'TypeError',
        message: /must be a finite number/
      });
    });
...
/**
 * Parses a query builder serialization into the nested object shape expected by
 * `@centralping/json-api-query` validation.
 *
 * @param {string} queryString - Output of `queryBuilder.toString()`.
 * @returns {object} - Parsed query object.
 */
function parseBuilderQueryString(queryString) {
  const result = Object.create(null);

  if (queryString.length === 0) return result;

  for (const [rawKey, value] of new URLSearchParams(queryString)) {
    const fieldsMatch = /^fields\[(.+)\]$/.exec(rawKey);
    if (fieldsMatch) {
      result.fields ??= Object.create(null);
      result.fields[fieldsMatch[1]] = value.split(',');
      continue;
    }

    const filterMatch = /^filter\[(.+)\]$/.exec(rawKey);
    if (filterMatch) {
      result.filter ??= Object.create(null);
      result.filter[filterMatch[1]] = value;
      continue;
    }

    const pageMatch = /^page\[(.+)\]$/.exec(rawKey);
    if (pageMatch) {
      result.page ??= Object.create(null);
      result.page[pageMatch[1]] = value;
      continue;
    }

    if (rawKey === 'include') {
      result.include = value.split(',');
      continue;
    }

    if (rawKey === 'sort') {
      result.sort = value.split(',');
      continue;
    }
...
describe('json-api-query schema validation', () => {
  const validator = validate();

  it('validates a full builder query string', () => {
    const q = createQueryBuilder()
      .fields('articles', ['title', 'body'])
      .include(['author'])
      .filter({published: true})
    ...
    const query = parseBuilderQueryString(q.toString());

    assert.equal(validator(query), true, JSON.stringify(validator.errors));
  });

  it('validates offset/limit page strategy output', () => {
    const q = createQueryBuilder().page({offset: 0, limit: 25});
    const query = parseBuilderQueryString(q.toString());

    assert.equal(validator(query), true, JSON.stringify(validator.errors));
  });

  it('validates cursor-only page strategy output', () => {
    const q = createQueryBuilder().page({cursor: 'opaque-token'});
    const query = parseBuilderQueryString(q.toString());

    assert.equal(validator(query), true, JSON.stringify(validator.errors));
  });

  it('validates custom parameters with non-lowercase names', () => {
    const q = createQueryBuilder().param('camelCase', 'value');
    const query = parseBuilderQueryString(q.toString());
