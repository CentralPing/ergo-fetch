...
describe('getPathPortion', () => {
  it('returns the full string when no delimiters are present', () => {
    assert.equal(getPathPortion('/items'), '/items');
  });

  it('strips query string', () => {
    assert.equal(getPathPortion('/items?page=2&limit=10'), '/items');
  });

  it('strips fragment', () => {
    assert.equal(getPathPortion('/items#section'), '/items');
  });

  it('strips at the first delimiter when both query and fragment are present', () => {
    assert.equal(getPathPortion('/items?page=2#section'), '/items');
    assert.equal(getPathPortion('/items#section?ignored'), '/items');
  });
});
