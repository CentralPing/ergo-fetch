...
describe('createIdempotencyInterceptor', () => {
  ...
  describe('request — body fingerprinting', () => {
    ...
    it('throws on fingerprint mismatch for body-then-no-body asymmetry', async () => {
      const interceptor = createIdempotencyInterceptor();

      const ctx1 = createMockCtx({
        idempotencyKey: 'asymmetric-key-1',
        body: JSON.stringify({order: 'A'})
      });
      await interceptor.request(ctx1);

      const ctx2 = createMockCtx({idempotencyKey: 'asymmetric-key-1'});
    ...
    it('throws on fingerprint mismatch for no-body-then-body asymmetry', async () => {
      const interceptor = createIdempotencyInterceptor();

      const ctx1 = createMockCtx({idempotencyKey: 'asymmetric-key-2'});
      await interceptor.request(ctx1);

      const ctx2 = createMockCtx({
        idempotencyKey: 'asymmetric-key-2',
        body: JSON.stringify({order: 'B'})
      });
