...
describe('createIdempotencyInterceptor', () => {
  ...
  describe('request — key generation', () => {
    ...
    it('generates a key for POST requests', async () => {
      ...
      assert.ok(key, 'expected idempotency-key header value');
    ...
    it('generates keys for configured methods', async () => {
      ...
      for (const ctx of [post, put, patch]) {
        const key = idempotencyHeaderValue(ctx.headers);
        assert.ok(key, 'expected idempotency-key header value');
        assert.equal(ctx.headers.get('idempotency-key'), `"${key}"`);
      }
  ...
  describe('request — custom header name', () => {
    it('uses the configured header name', async () => {
      ...
      const key = idempotencyHeaderValue(ctx.headers, 'x-idempotency-key');
      assert.ok(key, 'expected x-idempotency-key header value');
      assert.equal(ctx.headers.get('x-idempotency-key'), `"${key}"`);
      assert.equal(ctx.headers.has('idempotency-key'), false);
    });
  });
  ...
  describe('request — custom generator', () => {
    ...
    it('throws on fingerprint mismatch when generator collides with active explicit key', async () => {
      const interceptor = createIdempotencyInterceptor({generator: () => 'shared-key'});
      const explicit = createMockCtx({
        idempotencyKey: 'shared-key',
        body: JSON.stringify({order: 'A'})
      });
      await interceptor.request(explicit);

      const generated = createMockCtx({method: 'POST'});
      await assert.rejects(() => interceptor.request(generated), {
        name: 'TypeError',
        message: /fingerprint mismatch/
      });

      assert.equal(generated.headers.has('idempotency-key'), false);
    });
  });
  ...
  describe('request — RFC 8941 wire format', () => {
    ...
    it('throws TypeError for CTL characters in explicit key', async () => {
      ...
      await assert.rejects(() => interceptor.request(ctx), {
        name: 'TypeError',
        message: 'value must be an RFC 8941 sf-string value (visible ASCII)'
      });
    });
  ...
  describe('request — body fingerprinting', () => {
    ...
    it('rejects one of two parallel explicit-key requests with different bodies', async () => {
      const interceptor = createIdempotencyInterceptor();
      const ctx1 = createMockCtx({
      ...
      const rejectedOutcome = first.status === 'rejected' ? first : second;
      const rejectedCtx = first.status === 'rejected' ? ctx1 : ctx2;
      const fulfilledCtx = first.status === 'fulfilled' ? ctx1 : ctx2;

      assert.match(rejectedOutcome.reason.message, /fingerprint mismatch/);
      assert.equal(rejectedCtx.headers.has('idempotency-key'), false);
      assert.ok(fulfilledCtx.headers.has('idempotency-key'));
    });

    it('allows parallel explicit-key requests with identical bodies', async () => {
      const interceptor = createIdempotencyInterceptor();
      const body = JSON.stringify({order: 'A'});
      const ctx1 = createMockCtx({idempotencyKey: 'parallel-reuse-key', body});
      const ctx2 = createMockCtx({idempotencyKey: 'parallel-reuse-key', body});
      ...
      assert.equal(idempotencyHeaderValue(ctx1.headers), 'parallel-reuse-key');
      assert.equal(idempotencyHeaderValue(ctx2.headers), 'parallel-reuse-key');
    });

    it('allows explicit key reuse across bodiless body variants', async () => {
      const interceptor = createIdempotencyInterceptor();
      const key = 'bodiless-key';
      const bodilessBodies = [undefined, null, ''];

      for (const firstBody of bodilessBodies) {
        for (const secondBody of bodilessBodies) {
          const ctx1 = createMockCtx({idempotencyKey: key, body: firstBody});
          await interceptor.request(ctx1);
          assert.ok(ctx1.headers.has('idempotency-key'));
          assert.equal(idempotencyHeaderValue(ctx1.headers), key);

          const ctx2 = createMockCtx({idempotencyKey: key, body: secondBody});
          await assert.doesNotReject(() => interceptor.request(ctx2));
          assert.ok(ctx2.headers.has('idempotency-key'));
          assert.equal(idempotencyHeaderValue(ctx2.headers), key);

          interceptor.response(ctx1, createMockResponse({status: 200}));
        }
      }
    });
  ...
  describe('maxEntries — FIFO eviction', () => {
    it('evicts oldest auto-generated registry entry when capacity is exceeded', async () => {
      let call = 0;
      const interceptor = createIdempotencyInterceptor({
        maxEntries: 2,
        generator: () => `gen-key-${++call}`
      });

      const ctx1 = createMockCtx({method: 'POST'});
      const ctx2 = createMockCtx({method: 'POST'});
      const ctx3 = createMockCtx({method: 'POST'});

      await interceptor.request(ctx1);
      await interceptor.request(ctx2);
      await interceptor.request(ctx3);

      assert.equal(idempotencyHeaderValue(ctx1.headers), 'gen-key-1');
      assert.equal(idempotencyHeaderValue(ctx2.headers), 'gen-key-2');
      assert.equal(idempotencyHeaderValue(ctx3.headers), 'gen-key-3');
