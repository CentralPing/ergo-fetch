...
describe('createConditionalInterceptor', () => {
  describe('factory validation', () => {
    ...
    it('throws TypeError when read and write methods overlap', () => {
      assert.throws(
        () => createConditionalInterceptor({methods: {read: ['GET', 'PUT'], write: ['PUT']}}),
        {name: 'TypeError', message: 'methods.read and methods.write must not overlap: PUT'}
      );
    });
    ...
    it('throws TypeError when options is a number', () => {
      assert.throws(() => createConditionalInterceptor(1), {
        name: 'TypeError',
    ...
    it('throws TypeError when options is a string', () => {
      assert.throws(() => createConditionalInterceptor('test'), {
        name: 'TypeError',
        message: 'options must be an object'
      });
    });
    ...
    it('throws TypeError when options is a boolean', () => {
      assert.throws(() => createConditionalInterceptor(true), {
        name: 'TypeError',
        message: 'options must be an object'
      });
    });
    ...
    it('throws TypeError when options is null', () => {
      assert.throws(() => createConditionalInterceptor(null), {
        name: 'TypeError',
        message: 'options must be an object'
      });
    });
    ...
    it('throws TypeError when options is an array', () => {
      assert.throws(() => createConditionalInterceptor([]), {
        name: 'TypeError',
        message: 'options must be an object'
      });
    });
  ...
  describe('request', () => {
    ...
    it('sets If-None-Match for cached GET with ETag', async () => {
      await store.set('https://api.example.com/resource', {etag: '"abc123"', body: {id: 1}});
      const ctx = createMockCtx();
    ...
    it('sets If-Modified-Since for cached GET with Last-Modified', async () => {
      ...
      await store.set('https://api.example.com/resource', {lastModified, body: {id: 1}});
      const ctx = createMockCtx();
    ...
    it('sets If-None-Match with weak ETag for GET', async () => {
      await store.set('https://api.example.com/resource', {etag: 'W/"weak"', body: {id: 1}});
      const ctx = createMockCtx();
    ...
    it('does not set headers for GET when cache has no body', async () => {
    ...
    it('sets headers for HEAD even when cache has no body', async () => {
      await store.set('https://api.example.com/resource', {etag: '"v1"'});
      const ctx = createMockCtx({method: 'HEAD'});

      await interceptor.request(ctx);

      assert.equal(ctx.headers.get('if-none-match'), '"v1"');
    });
    ...
    it('respects custom read methods', async () => {
      ...
      await store.set('https://api.example.com/resource', {etag: '"tag"', body: {id: 1}});
  ...
  describe('response — caching', () => {
    ...
    it('does not cache for 204 No Content responses', async () => {
      const ctx = createMockCtx();
  ...
  describe('response — 304 handling', () => {
    ...
    it('returns undefined on 304 for HEAD (HEAD is bodyless)', async () => {
      await store.set('https://api.example.com/resource', {
        etag: '"v1"',
        body: {id: 1}
      });
